...
with st.expander("Calculation Breakdown"):
        ...
        st.markdown(f"- sin({theta_deg}°) = `{sin_theta:.3f}`")
        st.markdown(f"- cos({theta_deg}°) = `{cos_theta:.3f}`")
        st.markdown(f"**Step 2 — Multiply inside brackets:**")
        st.markdown(f"- 2 × sin(θ) × cos(θ) = `{denominator_trig:.3f}`")
        st.markdown(f"- Δd × g = {delta_d} × {g} = `{delta_d * g:.3f}`")
        st.markdown(f"**Step 3 — Divide (resolve bracket):**")
        st.markdown(f"- Δd·g / (2·sin·cos) = `{bracket:.3f}`")
        st.markdown(f"**Step 4 — Multiply by m, divide by k:**")
        st.markdown(f"- m × bracket / k = {m} × {bracket:.3f} / {k} = `{inside_sqrt:.3f}`")
        st.markdown(f"**Step 5 — Square root:**")
        st.markdown(f"- x = √{inside_sqrt:.3f} = **`{x:.3f} m`**")
